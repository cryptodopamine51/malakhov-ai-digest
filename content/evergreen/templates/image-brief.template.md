...
- Style: Malakhov AI Digest editorial — фотореалистичная деловая editorial photography, спокойный фактологический тон, без стокового глянца.
- Visual lane: перед написанием prompt'ов выбрать уникальные для статьи локацию, палитру, время суток, предметный словарь и камеру. Не использовать по умолчанию «тёмно-синий офисный стол с ноутбуком».
- Палитра: см. `docs/DESIGN.md`, но не повторять одну палитру во всех гайдах; цвет должен вытекать из темы.
- Запрещено внутри изображения: читаемый текст, watermarks, fake dashboards с надписями.
- Запрещённые сюжеты и стили: иллюстрации, изометрия, 3D-рендер, векторная графика, роботы, светящийся мозг, неон, рукопожатие человек+робот, generic office stock (люди в костюмах у монитора).
- Допустимо: реалистичные постановочные сцены, предметные раскладки, рабочие столы, документы без текста, цветовые маркеры без надписей, но cover и inline должны отличаться ракурсом и композицией.
...
Cover всегда генерируется в ChatGPT как фотореалистичная editorial photography, не как SVG.
