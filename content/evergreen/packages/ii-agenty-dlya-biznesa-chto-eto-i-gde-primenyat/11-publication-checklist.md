# Publication Checklist: ИИ-агенты для бизнеса: что это и где применять
...
- [ ] Canonical path is `/guides/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat`.
...
- [ ] Cover WebP exists at `/public/images/guides/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat/cover.webp`.
...
- [ ] `noindex: true` removed once `cover.webp` is in `public/images/guides/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat/` and `evergreen:check` is green.
- [ ] `npx tsx scripts/indexnow-batch.ts --apply` queued for post-deploy ping.

## Technical

- [ ] `content/guides/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat.md` exists for publication.
- [ ] `content/guides/meta/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat.json` exists and is valid JSON.
- [ ] `npm run evergreen:check -- --slug=ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat` passes.
