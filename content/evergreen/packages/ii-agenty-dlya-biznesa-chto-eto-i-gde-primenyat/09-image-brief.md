# Image Brief: ИИ-агенты для бизнеса: что это и где применять
...
- **Cover**: `ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat-cover.webp` (либо `<primary-keyword>-<short-modifier>.webp`, если slug длинный).
...
2. Владелец/редактор открывает ChatGPT, копирует prompt, генерирует PNG. Сохранять можно **с любым именем** (ChatGPT часто отдаёт `ChatGPT_image_<timestamp>.png` и подобное) — главное, чтобы все PNG для одной статьи лежали в `content/evergreen/packages/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat/raw-images/`.
3. Запускает `npm run images:prep -- --slug=ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat`. Скрипт:
...
4. `npm run evergreen:check -- --slug=ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat` проверяет наличие файлов и плотность cover (≥ 80 KB).
...
- `filename_png`: `ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat-cover.png` (SEO convention — см. секцию выше)
- `filename_webp`: `ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat-cover.webp`
- Финальный путь: `public/images/guides/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat/ii-agenty-dlya-biznesa-chto-eto-i-gde-primenyat-cover.webp`
