...
- **Per-source daily cap (2026-06-10):** `pipeline/claims.ts::claimBatch` перед клеймом
  применяет `applySourceDailyCap` — не более `SOURCE_DAILY_PUBLISH_CAP` (env, по умолчанию 10)
...
- `+1` если статья находится в canonical category `ai-russia` (`primary_category` или
  `secondary_categories`; legacy `topics` учитывается только как fallback для старых строк без
  canonical category). Раньше было `+2` и tag часто означал русскоязычный источник, а не российский
  рынок.
...
С 2026-06-10 кандидаты перед selection переранжируются `rankDigestCandidates` (score +
importance: масштаб события, известность игрока, мульти-источниковое подтверждение —
`lib/story-signal.ts::getStoryImportance`), поэтому «сочные» истории борются за слоты первыми,
...
- `npm run quality:judge` берёт ежедневную выборку (вчерашние Telegram channel posts +
  ...
  в `llm_usage_logs`.
- `npm run quality:feedback` отправляет владельцу в admin chat один нумерованный пост
  до 8 статей: вчерашние channel posts и худшие по judge. Inline-кнопки пишут
  `callback_data='af:<article_id>:<rating>'`.
- `app/api/tg-feedback/route.ts` принимает Telegram `callback_query`, проверяет
  `x-telegram-bot-api-secret-token` и разрешает callbacks от `TELEGRAM_OWNER_USER_ID`,
  `TELEGRAM_OWNER_USERNAME` либо из чата `TELEGRAM_ADMIN_CHAT_ID`, upsert-ит `article_feedback`
  и помечает оценённую строку подтверждением `✓ ...`.
...
- `pipeline/ingest.ts` использует `lib/categories.ts::resolveArticleCategories`: feed-level `topics`
  нормализуются перед записью в `articles.primary_category`, `articles.secondary_categories` и
  legacy `articles.topics`. `ai-russia` — это содержательный сигнал про российский рынок/компании/
  регуляторов, а не синоним русскоязычного источника. Если RSS-фид российского медиа пометил статью
  `ai-russia`, но в title/snippet нет Russia-market сигнала, tag удаляется; при отсутствии других
  категорий article падает в `ai-industry`, а у Habr AI — в оставшийся `coding`.
- Поле `articles.topics` сохраняется как legacy fallback до полного cutover в волне 2 для возможности отката. Новые reads и scoring должны опираться на `primary_category` / `secondary_categories`; новые ingest-записи пишут туда уже нормализованный список категорий.
...
**Cover mirroring (2026-06-10).** Внешние (hotlink) cover-URL при apply зеркалятся в R2:
`prepareEditorialApplication` → `pipeline/cover-mirror.ts::mirrorCoverToR2` скачивает картинку
(timeout 15s, max 10 MB), конвертирует в WebP 1200w и заливает
`article-images/mirrored-covers/<articleId>.webp` + варианты `-400`/`-800`
(`lib/r2-images.ts::uploadWebpWithVariants`). На сайте такие обложки отдаются нативным
`<img srcset>` (см. Responsive cover variants), минуя `/_next/image`. Любой сбой — мягкий
fallback на исходный внешний URL; в cron-окружениях без `R2_*` env зеркало no-op.
Backfill существующего корпуса — `scripts/mirror-covers-to-r2.ts` (прогон 2026-06-10:
mirrored=913, skipped=6 из 919 live). Сегмент `mirrored-covers` сознательно не матчится
паттерном `(ai|template|stock)-covers`: для scorer и `needsAiCover` зеркало остаётся
«настоящей» обложкой источника.
