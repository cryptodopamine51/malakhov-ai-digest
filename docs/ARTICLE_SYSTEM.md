...
- Featured-карточки с cover-изображениями рендерят текстовый контент внутри контрастной плашки
  поверх затемнённого изображения. Нельзя полагаться только на белый текст и gradient overlay:
  исходные covers часто являются скриншотами или картинками с крупным текстом.
