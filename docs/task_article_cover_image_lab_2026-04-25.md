...
Каждый локальный режим выбирает композицию детерминированно от статьи: title/source/topic превращаются в seed,
по которому подбираются палитра, motif variant, glow position и короткая текстовая метка. Это нужно, чтобы несколько
статей одного топика не выглядели одинаковыми в лаборатории.

Это сделано специально как frontend visual test, а не как production pipeline.

Обновление после первого прод-просмотра:

- stock mock теперь выбирает не одну картинку на топик, а вариант из небольшого пула по seed статьи;
- stock editorial treatment варьирует crop, overlay, frame и paper block;
- `Local SVG`, `Cover bank` и `AI budget` получили несколько разных композиций, чтобы по одной странице можно было
  сравнить разные направления, а не одну повторяющуюся обложку.
