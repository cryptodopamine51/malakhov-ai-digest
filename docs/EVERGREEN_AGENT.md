...
4. **Один развёрнутый кейс** по структуре *Ситуация → Что делает ИИ → Что нужно для пилота → Метрики → Итог*. Источник: реальный публичный → анонимизированный → редакционный сценарий. В опубликованном тексте не писать «Редакционный пример», не добавлять source-note под кейсом и не называть кейс нашим/клиентским без основания. Заголовок H3 начинается с «Кейс:» или «Сценарий:» или «Ситуация:» или «Мини-кейс:».
...
- `prompt` (4–8 строк для ChatGPT, фотореалистичная editorial photography + конкретная сцена + ограничения);
- `negative_prompt` (no illustration, no isometric art, no 3D render, no vector graphics, no robots, no neon, no glowing brain, no handshake, no readable text inside image, no generic office stock);
...
Cover **обязателен** (canonical 1200×675 WebP, ≥ 50 KB финал) и по умолчанию должен выглядеть как реалистичная редакционная фотография. Inline — обычно 2–4 шт. (1200×800 или 1200×1200): ChatGPT-картинки тоже делать в фотореализме; если нужна строгая матрица, схема или roadmap, лучше использовать локальный SVG/WebP как отдельное осознанное исключение. `images:prep` также создаёт `-480.webp` и `-768.webp` variants для каждого local guide image.
...
Привет. Сгенерируй N фотореалистичных картинок для статьи «<title>».
Стиль: реалистичная editorial photography для делового медиа.
...
[Финальные имена WebP, размеры, cover ≥ 50 KB, `-480/-768` variants.]
```
...
- конвертирует canonical в WebP quality cover q88 / inline q88;
- пишет canonical в `public/images/guides/<slug>/<filename>.webp`;
- пишет responsive variants `public/images/guides/<slug>/<filename>-480.webp` и `...-768.webp`;
...
Допустимы только warnings, никаких errors. `cover_min_size` warning **не должен** оставаться (мы только что положили cover из ChatGPT — он обязан быть ≥ 50 KB; если меньше — попросить владельца перегенерить).
...
Картинки:
- <cover-filename>.webp (1200×675, <размер KB>) + -480/-768 variants
- <filename>.webp (1200×800, <размер KB>) + -480/-768 variants
- […]
...
| Cover < 50 KB после конвертации | Попросить владельца перегенерить cover в ChatGPT с более насыщенной композицией; не публиковать |
