...
Production default — `TELEGRAM_WEEKLY_REPORT_FORMAT=signal`; допустимы
...
вроде «без шума» в недельный отчёт не добавляются. Вступительной сводки перед списком нет,
а пояснение каждой новости обрезается только по границе законченного предложения.
