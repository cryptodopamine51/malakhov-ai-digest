...
| `quality-feedback.yml` | ежедневно 08:15 UTC / 11:15 МСК | LLM-judge выборки статей и one-tap feedback в admin chat |
...
2. `npm run quality:feedback` — отправляет владельцу карточки в `TELEGRAM_ADMIN_CHAT_ID` с
   кнопками `🔥 сильная` / `👌 норм` / `👎 слабая`.
