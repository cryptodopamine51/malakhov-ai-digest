...
2. `npm run quality:feedback` — отправляет владельцу один нумерованный пост в
   `TELEGRAM_ADMIN_CHAT_ID` с кнопками `🔥 сильная` / `👌 норм` / `👎 слабая` для каждой статьи.
...
и помечает оценённую строку подтверждением. Повторный тап перезаписывает оценку.
