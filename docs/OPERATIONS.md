...
Telegram-посты дёргаются через Supabase `pg_cron` + `pg_net`. Каждый cron вызывает один slot:
...
пяти точных отправок в день; primary schedule — Supabase `pg_cron`. Route
`/api/cron/tg-channel-post?slot=N` остаётся Vercel serverless endpoint-ом, но вызывается из
Postgres через `pg_net`.
...
- `Что отправилось / что нет` — отдельно показывает статус Telegram channel posts,
...
- `Что нужно для зелёного` — появляется при `🟡`/`🔴` и перечисляет конкретные
  ...
  восстановить Telegram/live-публикации и т.п.
