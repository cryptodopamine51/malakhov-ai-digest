...
`NEXT_PUBLIC_R2_IMAGE_VARIANTS` (`on` | unset) — фича-флаг адаптивных cover-вариантов (см.
секцию «Responsive cover variants» ниже). По умолчанию **выключен**. `NEXT_PUBLIC_R2_PUBLIC_BASE_URL`
нужен только при переезде R2 на custom domain (host ≠ `*.r2.dev`): тогда клиентский
`isR2ImageUrl` должен знать новый host. Оба — `NEXT_PUBLIC_*`, т.е. инлайнятся в клиентский
бандл на этапе build, и для их применения нужен **редеплой**.
...
1. Прогнать `scripts/backfill-cover-variants.ts` до `failed=0` — у каждой R2-обложки должны
   появиться варианты. Инвариант: пока флаг включён, отсутствие варианта = 404 на выбранный
   браузером кандидат (`-400`/`-800`).
2. Выставить `NEXT_PUBLIC_R2_IMAGE_VARIANTS=on` в Vercel env и **редеплой** (это `NEXT_PUBLIC_*`,
   инлайнится в бандл на build).
3. Замерить LCP до/после на главной и странице статьи (PageSpeed Insights / Lighthouse).
4. Откат — снять флаг + редеплой; варианты в R2 остаются, base-URL'ы не менялись.
...
- секреты workflow: `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, `TELEGRAM_BOT_TOKEN`,
  `TELEGRAM_ADMIN_CHAT_ID`; `SITE_MONITOR_URL` — опциональная repo variable.
- pure-логика решения покрыта `tests/node/site-feed-monitor.test.ts`.
...
последний ingest/enrich, digest status за сегодня, группировка open alerts, source
rejected/fetch errors и LLM/image cost за московский день.

Расписание workflow `.github/workflows/ops-report.yml`:

| Cron UTC | МСК | Назначение |
|---|---|---|
| `45 6 * * 1-5` | 09:45 Пн–Пт | Утренний отчёт после weekday digest |
| `45 8 * * 6,0` | 11:45 Сб–Вс | Утренний отчёт после weekend digest |
| `30 17 * * *` | 20:30 ежедневно | Вечерний отчёт по насыщению дня |

Формат начинается со светофора и человекочитаемого операторского блока:

- `🟢` — ingest/enrich/digest/live/cost без существенных проблем;
- `🟡` — есть warning alerts, open batches, старый pending, низкое насыщение или source failures;
- `🔴` — есть critical alerts, failed ingest/enrich/digest или за день нет live-публикаций.

Первые секции отчёта намеренно написаны обычным русским языком для администратора:

- `Итог` — коротко объясняет цвет: зелёный = всё ок, жёлтый = портал работает,
  но есть предупреждения, красный = критическая проблема;
- `Что отправилось / что нет` — отдельно показывает статус Telegram-дайджеста,
  live-публикаций на сайте и потерь доставки;
- `Проблемы` — расшифровывает причины вроде `batch_poll_stuck` и
  `claude_parse_failed` как понятные действия/симптомы, без необходимости помнить
  внутренние alert type.
- `Что нужно для зелёного` — появляется при `🟡`/`🔴` и перечисляет конкретные
  условия, которые надо закрыть: дособрать batch, разобрать warning/critical,
  восстановить digest/live-публикации и т.п.
- `Промпт для Codex` — появляется при `🟡`/`🔴` как HTML `<pre>` block, чтобы его
  можно было нажать и скопировать в локального агента. Внутри уже есть путь к
  репозиторию, причины сигнала, ключевые метрики и стартовая команда dry-run.

Технические метрики (`Публикации`, `Пайплайн`, `Расход`, `Алерты`) остаются ниже
для диагностики. Warning/info по умолчанию не присылаются отдельными сообщениями:
они копятся в `pipeline_alerts` и объясняются в утренне-вечерней ops-сводке.
