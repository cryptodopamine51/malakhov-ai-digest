...
rejected/fetch errors и LLM/image cost за московский день. Если настроены
...
`Минимум цифр`, `Что делать`. Технические подробности не разворачиваются в каждом
сообщении: `/alerts` и `/cost` остаются отдельными admin-only командами.
