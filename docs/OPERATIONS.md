...
| `retry-failed.yml` | каждые 4 часа, на 30 минуте | возврат retryable статей; повторный enrich с Claude budget `$2` |
...
- Порог budget guard задаётся через `CLAUDE_DAILY_BUDGET_USD`; в workflow `enrich.yml`, `retry-failed.yml` и `pipeline-health.yml` стоит `$2` (с 2026-05-05 — фактический расход 1.0–1.4/день при прежнем `$1` стабильно резал submit). Default в коде остаётся `$1`. Перед повышением свериться с `npm run cost:report`.
