...
- **Поток:** утром бот присылает владельцу до 8 сообщений: вчерашние 5 channel-постов
  (заголовок + ссылка + cover-превью) и 2–3 худшие статьи по judge («judge считает слабой:
  …»). У каждого — inline-кнопки `🔥 сильная` / `👌 норм` / `👎 слабая`.
