...
Сделано агентом (responsive image delivery 2026-06-17):
- **Guide images без Vercel optimizer**: `images.unoptimized: true` остаётся инвариантом.
  `scripts/images-prep.ts` теперь генерирует canonical WebP + local variants `-480.webp` /
  `-768.webp` через существующий `sharp` pipeline. Published guides backfill прогнан.
- **Native responsive rendering**: local guide surfaces (`/`, `/guides`, `/guides/<slug>`) используют
  `ResponsiveLocalImage` с `<picture>`/`srcset`/`sizes`; mobile source ограничен 480/768w, чтобы
  high-DPR телефоны через VPN не выбирали canonical 1200w.
- **Cache + gates**: `/images/guides/*` получает `Cache-Control: public, max-age=604800,
  stale-while-revalidate=2592000`; `evergreen:check` валидирует наличие/размеры/бюджеты variants,
  добавлен `npm run images:audit`.
- **Article R2 variants**: R2 `-400`/`-800` считаются обязательным invariant, env-флаг
  `NEXT_PUBLIC_R2_IMAGE_VARIANTS` убран; article cards/hero всегда отдают доступный R2 `srcset`.
