...
- **Открытый блокер (решение владельца):** Supabase разблокировать ожиданием сброса egress-квоты или временным апгрейдом до Pro. Без этого миграция обложек и оживление pipeline невозможны. После разблокировки: `npx tsx scripts/migrate-covers-to-r2.ts --dry-run` → `--apply`, затем re-run упавших workflow'ов.
