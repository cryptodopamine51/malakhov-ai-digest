...
Открыто: B5→A4 (письмо GetPayAll, отправляет владелец).
Дополнение 2026-06-12 (**evergreen burst #2**, коммиты `88d8e04`+`ae0e22f`): **6 гайдов
опубликованы в прод под `noindex: true` с плейсхолдер-обложками** (решение владельца —
«статьи без картинок noindex»): id=8 «Чем ИИ-агент отличается от чат-бота»,
id=10 «ИИ-агенты в поддержке клиентов», id=13 «Нейросети для маркетинга: 25 сценариев»
(первый в кластере «Маркетинг и контент»), id=19 «Vibe coding» (первый в кластере AI coding),
id=34 «Локальные нейросети 2026», id=35 «Perplexity в России» (новая тема — **B6 шорт-лист
закрыт полностью**, п.4+п.6). Прод проверен: 200 + noindex на всех шести.
Снятие noindex: владелец генерит PNG по `12-chatgpt-image-prompts.md` пакета →
«картинки положил, продолжай» → `images:prep` (перепишет плейсхолдеры по тем же именам) →
noindex off → topics.json `ready_for_codex`→`published` → deploy + IndexNow.
Стиль-инсайт для будущих статей: `evergreen:check` блокирует «не X, а Y» и термин no-code —
писать сразу без них.
