...
Дополнение 2026-06-12 (**evergreen burst #2**, коммит `88d8e04`): три полных пакета
`ready_for_codex`, ждут PNG от владельца (промпты — `12-chatgpt-image-prompts.md` каждого пакета):
id=10 «ИИ-агенты в поддержке клиентов», id=13 «Нейросети для маркетинга: 25 сценариев»,
id=34 «Локальные нейросети 2026» (новая тема, Вордстат 6 217 — закрывает B6 п.4 шорт-листа).
После «картинки положил, продолжай»: `images:prep` → перенос в `content/guides/` → noindex off →
checks → deploy (шаги в `10-codex-publication-task.md` пакета). Из шорт-листа B6 остался
п.6 «Perplexity из России» (1 429/мес, вторая волна).
